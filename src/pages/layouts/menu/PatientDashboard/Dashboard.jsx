...
import { CircleUserRound, Heart, Users, ShieldCheck, ClipboardCheck } from 'lucide-react';
...
function App() {
  ...
  const getSectionCompletionStatus = () => {
    return {
      ...
      insurance: Boolean(userData.insurance.provider && userData.insurance.policyNumber)
    };
  };
  ...
  const showFeedback = (message, type = 'success') => {
    ...
    setTimeout(() => {
      setFeedbackMessage({ show: false, message: '', type: '' });
    }, 3000);
  };
  ...
  const handleDeleteFamilyMember = async (index) => {
    const memberToDelete = userData.familyMembers[index];
    setUserData(prev => ({
      ...prev,
    ...
    try {
      await axios.delete(`https://6808fb0f942707d722e09f1d.mockapi.io/FamilyData/${memberToDelete.id}`);
  ...
  const getIcon = (iconName) => {
    switch (iconName) {
      case 'user':
        return <CircleUserRound className="w-5 h-5 sm:w-6 sm:h-6" />;
      case 'heart':
  ...
  className="bg-[#F4C430] text-[#0e1630] font-bold px-4 sm:px-6 py-2 sm:py-3 rounded-lg text-sm sm:text-base hover:scale-105 transition"
>
          ...
          return (
            <button 
              ...
              className={`px-4 py-2 rounded-lg flex items-center gap-2 text-white text-sm sm:text-base
                ${isActive 
                  ? 'bg-[#0e1630]' 
                  : 'bg-[#1f2a4d] hover:bg-[#1b264a]'
                } transition-all duration-300`}
            >
              {getIcon(section.icon)} 
              {section.name}
              {isCompleted && (
                <ClipboardCheck className="text-green-400 ml-1 animate-pulse" />
              )}
            </button>
          );
        })}
            ...
            {activeSection === 'personal' && (
              <form onSubmit={handlePersonalHealthSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input 
                    name="surgeries" 
                    defaultValue={userData.surgeries}
                    className="w-full border p-2 rounded" 
                  />
                </div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox" 
                      name="isSmoker" 
                      defaultChecked={userData.isSmoker}
                    /> 
                    Do you smoke?
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox" 
                      name="isAlcoholic" 
                      defaultChecked={userData.isAlcoholic}
                    /> 
                ...
                <div className="flex justify-end gap-4 mt-6">
                  ...
                  <button 
                    type="submit" 
                    className="px-4 py-2 rounded-lg bg-[#0e1630] text-white"
                  >
                    Save
                  </button>
                </div>
                      ...
                      {[
                        ...
                        <label key={disease} className="flex items-center gap-2 text-sm">
                          <input
                            type="checkbox"
                            value={disease}
                            checked={familyInput.diseases.includes(disease)}
                  ...
                  <div className="mt-6 space-y-3">
                    ...
                    <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                      {userData.familyMembers.map((member, index) => (
                        <div 
                          key={index} 
                          className="p-4 bg-gray-100 rounded-lg shadow-sm flex justify-between items-start transition-all hover:shadow-md"
                        >
                          <div>
                            ...
                            {member.number && (
                              <p className="text-sm text-gray-600">
                                <span className="font-medium">Phone:</span> {member.number}
                              </p>
                            )}
                            ...
                            {member.diseases.length > 0 && (
                              <p className="text-sm text-gray-600 mt-1">
                                ...
                                <span className="ml-1">{member.diseases.join(', ')}</span>
                              </p>
                            )}
                          </div>
              ...
              <form onSubmit={handleInsuranceSubmit} className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <select 
                      name="coverageType" 
                      defaultValue={userData.insurance.coverageType}
                      className="w-full border p-2 rounded" 
                      required
                    >
                  ...
                  <div>
                    ...
                    <input 
                      name="coverageAmount" 
                      defaultValue={userData.insurance.coverageAmount}
                      className="w-full border p-2 rounded" 
                      required 
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      name="startDate" 
                      type="date" 
                      defaultValue={userData.insurance.startDate}
                      className="w-full border p-2 rounded" 
                      required 
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      name="endDate" 
                      type="date" 
                      defaultValue={userData.insurance.endDate}
                      className="w-full border p-2 rounded" 
                      required 
                    />
                  </div>
                  ...
                  <div className="sm:col-span-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input 
                        type="checkbox" 
                        name="primaryHolder" 
                        defaultChecked={userData.insurance.primaryHolder}
                      /> 
