...
const DashboardOverview = () => {
  ...
  useEffect(() => {
    ...
    const fetchHealthSummary = async () => {
      try {
        const response = await axios.get(
          "https://681903225a4b07b9d1d1ae72.mockapi.io/health-summary"
        );
        ...
        if (data.length > 0) {
          ...
          setNewVitals({
            ...
            bloodSugar: first.bloodSugar || "",
            bloodPressure: first.bloodPressure || "",
          });
        }
      } catch (error) {
        console.error("Health summary fetch error", error);
    ...
    try {
      if (isNew) {
        ...
        const response = await axios.put(
          `https://681903225a4b07b9d1d1ae72.mockapi.io/health-summary/${summaryId}`,
          vitalsToSave
        );
        setHealthSummary(response.data);
      }
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center pt-20 z-50" onClick={() => setShowModal(false)}>
          <div className="bg-white text-[#0e1630] p-6 rounded-xl shadow-2xl w-full max-w-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end mt-6 gap-3">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 border border-gray-400 rounded-md hover:bg-gray-100">
                Cancel
              </button>
              <button onClick={saveHealthSummary} className="bg-green-500 text-white px-4 py-2 rounded">
                {isNew ? "Save" : "Update"}
              </button>
            </div>
          </div>
