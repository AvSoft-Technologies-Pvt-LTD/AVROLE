  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'isSameAsPermanent') {
      setFormData((prev) => ({
        ...
        temporaryAddress: val ? prev.permanentAddress : '',
      }));
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="relative flex flex-col gap-2">
              ...
              {formErrors.photo && <p className="text-xs text-red-500">{formErrors.photo}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 gap-3">
            {['password', 'confirmPassword'].map((field, index) => (
              <div className="relative space-y-1" key={index}>
                ...
                />
                {/* <span onClick={() => setShowPassword(!showPassword)} className="absolute top-3 right-2 cursor-pointer text-gray-700">
          ...
          {formErrors.agreeDeclaration && <p className="text-red-500 text-xs">{formErrors.agreeDeclaration}</p>}

          <div className="flex justify-center">
            <button type="submit" disabled={loading} className="bg-[#0e1630] hover:bg-[#F4C430] text-white py-2 px-6 rounded-lg shadow-lg w-full md:w-auto transition">
              {loading ? "Submitting..." : "Verify & Proceed"}
            </button>
          </div>
